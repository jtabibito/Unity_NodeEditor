    ...
    public class DataAsset
    {
        ...
        public Task<List<NodeComponent>> LoadGraph()
        {
            Task<List<NodeComponent>> t = new Task<List<NodeComponent>>(() => {
                ...
                using (FileStream fs = new FileStream(".\\Assets\\Editor\\Export\\" + m_strDataSource + "_Graph.ui", FileMode.OpenOrCreate, FileAccess.Read))
        ...
        public Task<Dictionary<int, DataBase>> LoadData()
        {
            Task<Dictionary<int, DataBase>> t = new Task<Dictionary<int, DataBase>>(() => {
                ...
                using (FileStream fs = new FileStream(".\\Assets\\Editor\\Export\\" + m_strDataSource + "_Data.data", FileMode.OpenOrCreate, FileAccess.Read))
                using (StreamReader sr = new StreamReader(fs))
                {
                    try
                    {
        ...
        public async Task<List<NodeComponent>> Load()
        {
            ...
            foreach (var pNode in listNodes)
            {
               ...
               {
                   pNode.m_pScript = UnityEditor.EditorGUIUtility.Load("Assets/Editor/Data/" + pNode.m_pData.GetType() + ".cs") as UnityEditor.MonoScript;
               }
            }
            return listNodes;
        }
        ...
        {
            Task t = new Task(() => {
                using (FileStream fs = new FileStream(".\\Assets\\Editor\\Export\\" + m_strDataSource + "_Graph.ui", FileMode.Create, FileAccess.Write))
        ...
        {
            Task t = new Task(() => {
                using FileStream fs = new FileStream(".\\Assets\\Editor\\Export\\" + m_strDataSource + "_Data.data", FileMode.Create, FileAccess.Write);
                using StreamWriter sw = new StreamWriter(fs);
                StringBuilder sb = new StringBuilder();
